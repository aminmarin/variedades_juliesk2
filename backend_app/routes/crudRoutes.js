...
module.exports = (db) => {
   ...
   //curl -X DELETE http://localhost:5000/crud/deleteProducto/1
    ...
    router.post('/createProducto', (req, res) => {
        ...
        const values = [nombre, descripcion, categoria, precio, stock];
    ...
    router.put('/updateProducto/:id_producto', (req, res) => {
        ...
        const values = [nombre, descripcion, categoria, precio, stock, id_producto];
    ...
    router.get('/readClientes', (req, res) => {
      ...
      // sentencias para el crud de la tabla categorias
        //curl http://localhost:5000/crud/readCategorias
        //curl -X POST -H "Content-Type: application/json" -d "{\"idCategoria\":1,\"nombre\":\"Niño\"}" http://localhost:5000/crud/createCategoria
        //curl -X PUT -H "Content-Type: application/json" -d "{\"nombre\":\"Niña\"}" http://localhost:5000/crud/updateCategoria/1
        //curl -X DELETE http://localhost:5000/crud/deleteCategoria/1

        //ruta para leer los registros de la tabla categorias
        router.get('/readCategorias', (req, res) => {
          ...
          db.query(sql, (err, result) => {
            if (err) {
              console.error('Error al leer registros de categorias:', err);
              res.status(500).json({ error: 'Error al leer registros de categorias' });
            } else {
              // Devolver los registros en formato JSON como respuesta
              res.status(200).json(result);
            }
        ...
        //ruta para obtener un registro de la tabla categorias
        router.get('/readCategorias/:idCategoria', (req, res) => {
          // Obtén el ID del registro a actualizar desde los parámetros de la URL
        ...
        //ruta para insertar un nuevo registro en la tabla categorias
        router.post('/createCategoria', (req, res) => {
          ...
          // Realiza la consulta SQL para insertar un nuevo registro con ID específico
          const sql = `INSERT INTO categoria (id_categoria,nombre) VALUES (?, ?)`;
          const values = [id_categoria,nombre];
          ...
          db.query(sql, values, (err, result) => {
            if (err) {
              ...
              res.status(201).json({ message: 'Categoria registrada con éxito' });
            }
          });
        });

        //ruta para actualizar un registro de la tabla categorias
        ...
        //ruta para eliminar un registro de la tabla categorias
